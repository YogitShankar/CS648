...
bool is_acute_angled(Point p1, Point p2, Point p3) {
    ...
    double longestSide = std::max({side1, side2, side3});
    double other1 = std::min({side1, side2, side3});
    double other2 = side1 + side2 + side3 - longestSide - other1;

    return (other1 * other1 + other2 * other2) + 1e-8 >= longestSide * longestSide;
}
...
Circle algo_n3(vector<Point> & points, int n){
    Circle C = {0,0,1e15};
    ...
    for(int i = 0; i < n; i++){
        for(int j = i+1; j < n; j++){
            Circle C1 = make_circle(points[i], points[j]);
            for(int k = j+1; k < n; k++){ // For every nC2 points we find the third point by finding largest acute angled circumcircle
                Circle C2 = make_circle(points[i], points[j], points[k]);
                if(is_acute_angled(points[i], points[j], points[k]) && C2.r > C1.r){
                    C1 = C2;
                }
            }
            int flag = 1; // Check if all points lie inside or not
